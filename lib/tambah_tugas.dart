// lib/screens/tambah_tugas_screen.dart
...
class TambahTugasScreen extends StatefulWidget {
  // Opsional: Jika widget ini menerima data tugas, maka ia akan masuk ke mode edit.
...
class _TambahTugasScreenState extends State<TambahTugasScreen> {
  ...
  // Daftar matkul (bisa juga diambil dari Firestore agar dinamis)
  final List<String> _matkulList = ['Biologi', 'Fisika', 'Kimia', 'Matematika', 'Sejarah', 'Geografi'];
  ...
  Future<void> _pilihDeadline(BuildContext context) async {
    final DateTime? picked = await showDatePicker(
      ...
      firstDate: DateTime.now(), // Deadline tidak bisa di masa lalu
  ...
  Future<void> _simpanTugas() async {
    ...
    if (_selectedDeadline == null) {
       ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Deadline tugas harus diisi!'), backgroundColor: Colors.red),
      );
      return;
    }
    ...
    try {
      if (_isEditMode) {
        // --- LOGIKA UPDATE TUGAS ---
        await FirebaseFirestore.instance.collection('tugas').doc(widget.tugasToEdit!.id).update({
        ...
        // TODO: Handle logic untuk menjadwal ulang notifikasi jika deadline berubah
        
        if (mounted) {
           ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Tugas berhasil diperbarui!'), backgroundColor: Colors.green),
          );
          Navigator.pop(context); // Kembali setelah update
        }

      } else {
        // --- LOGIKA TAMBAH TUGAS BARU ---
        ...
        await _notificationService.scheduleDailyNotification(
          notificationId, 'Jangan Lupa Tugas SinauApp!',
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Gagal menyimpan tugas: $e'), backgroundColor: Colors.red),
        );
      }
    } finally {
          ...
          child: Column(
            ...
            children: [
              ...
              DropdownButtonFormField<String>(
                ...
                onChanged: (newValue) => setState(() => _selectedMatkul = newValue),
                validator: (value) => value == null ? 'Mata kuliah harus dipilih' : null,
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      Text(
                        _selectedDeadline == null ? 'Pilih tanggal' : DateFormat('EEEE, d MMMM yyyy').format(_selectedDeadline!),
                        style: TextStyle(color: _selectedDeadline == null ? Colors.grey[700] : Colors.black, fontSize: 16),
                      ),
              ...
              TextFormField(
                ...
                decoration: _inputDecoration(hint: 'Jelaskan detail tugas di sini...'),
              ),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: _isLoading
                      ...
                      : Text(_isEditMode ? 'Update' : 'Simpan', style: const TextStyle(fontSize: 18, color: Colors.white, fontWeight: FontWeight.bold)),
                ),
              )
            ],
          ),
