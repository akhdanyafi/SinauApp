...
class _HomeScreenState extends State<HomeScreen> {
  // ... (semua fungsi _toggleTaskStatus, _deleteTask, _navigateToDetail tetap sama)
  ...
  void _deleteTask(DocumentSnapshot doc) {
    final data = doc.data() as Map<String, dynamic>;
    final int notificationId = data['notificationId'] ?? 0;

    FirebaseFirestore.instance.collection('tugas').doc(doc.id).delete().then((
      _,
    ) {
      if (notificationId != 0) {
        _notificationService.cancelNotification(notificationId);
      ...
      _notificationService.showNotification(
        'Tugas Dihapus',
        'Tugas ${data['nama_matakuliah']} telah dihapus.',
      );
    });
  }
    ...
    return Scaffold(
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          List<DocumentSnapshot> tugasList = [];
          if (snapshot.hasData) {
            tugasList = snapshot.data!.docs;
          }
          ...
          // --- Filtering Logic ---
          final tugasLewatDL = tugasList.where((t) {
            final deadline = (t['deadline_tugas'] as Timestamp).toDate();
            return deadline.isBefore(today) && !(t['dikerjakan'] as bool);
          }).toList();

          final tugasMepetDL = tugasList.where((t) {
            final deadline = (t['deadline_tugas'] as Timestamp).toDate();
            return !deadline.isBefore(today) &&
                deadline.isBefore(threeDaysFromNow) &&
                !(t['dikerjakan'] as bool);
          }).toList();

          // --- PENAMBAHAN LOGIKA BARU ---
          final tugasAkanDatang = tugasList.where((t) {
            final deadline = (t['deadline_tugas'] as Timestamp).toDate();
            return !deadline.isBefore(threeDaysFromNow) &&
                !(t['dikerjakan'] as bool);
          }).toList();

          final tugasSaatIni = tugasList
              .where((t) => !(t['dikerjakan'] as bool))
              .toList();
          final tugasSelesai = tugasList
              .where((t) => (t['dikerjakan'] as bool))
              .toList();
          ...
          for (var tugas in tugasSaatIni) {
            ...
            final deadlineDate = DateTime.utc(
              deadline.year,
              deadline.month,
              deadline.day,
            );
          ...
          return ListView(
            ...
            children: [
              ...
              if (tugasSelesai.isNotEmpty)
                ...tugasSelesai.map(
                  (tugas) =>
                      _buildTugasCard(tugas, const Color(0xFF34D399), true),
                )
              else
              ...
              if (tugasMepetDL.isNotEmpty)
                ...tugasMepetDL.map(
                  (tugas) =>
                      _buildTugasCard(tugas, const Color(0xFFFBBF24), false),
                )
              else
                _buildEmptyStateCard(
              ...
              // --- PENAMBAHAN BAGIAN BARU UNTUK MENAMPILKAN TUGAS LAINNYA ---
              const SizedBox(height: 24),
              const Text(
                'Tugas Akan Datang',
                style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold),
              ...
              if (tugasAkanDatang.isNotEmpty)
                ...tugasAkanDatang.map(
                  (tugas) =>
                      _buildTugasCard(tugas, const Color(0xFF60A5FA), false),
                )
              else
  ...
  // ... (semua widget _buildSummaryCard, _buildTugasCard, _buildEmptyStateCard tetap sama)
